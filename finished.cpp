...
void initTokenMap() {
    ...
    vector<string> keywordList = { "int", "void", "char","break", "float", "while", "do", "struct", "const", "case", "for", "return", "if", "default","else", "goto", "functioncall"};
            ...
            if (grammar.parseTable[top].find(currentToken) != grammar.parseTable[top].end())
            {
                ...
                if (rule != "ε") 
                {
                    ...
                    for (int i = symbols.size() - 1; i >= 0; --i)
                    {
                        ...
                        cout << "Number" << currentTokenIndex << ":" << currentToken << " Rule: " << top << "->" << rule << " Push {" << symbols[i] << "} rest:" << parseStack.size() << "with i = " << i << "\n";
                    }
                }
            }
            else 
